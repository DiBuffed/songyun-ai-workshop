...
def get_share_link() -> str:
    """Get the public share URL. Prefer live Gradio link when running, else HF Space (24/7)."""
    if PUBLIC_LINK_FILE.exists():
        try:
            url = PUBLIC_LINK_FILE.read_text(encoding="utf-8").strip()
            if url and "gradio.live" in url:
                return url  # Live Gradio tunnel (app running)
        except Exception:
            pass
    if CONFIG.get("hf_space_url"):
        return CONFIG["hf_space_url"]  # 24/7 HF Space fallback
    return ""
    ...
    with gr.Blocks(title=cfg.get("app_title", "Songyun AI Workshop / 宋韵")) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Group(elem_classes=["section-card"]):
                    gr.HTML(
                        '<div class="section-title">External Share Link</div>'
                    )
                    ...
                    if not share_msg:
                        share_msg = "Run via run.bat → Gradio link appears in popup & PUBLIC_LINK.txt"
                    ...
                    hf_link = CONFIG.get("hf_space_url", "")
                    if hf_link:
                        gr.HTML(
                            f'<div style="font-size:0.75rem;color:#64748b;margin-top:0.5rem;">24/7 (no PC): <a href="{hf_link}" target="_blank">{hf_link}</a></div>'
                        )
                    def refresh_share_link():
                        ...
                        return "Run via run.bat → Gradio link in popup & PUBLIC_LINK.txt"
...
def main():
    ...
    # Railway, Render: no share tunnel, direct HTTPS
